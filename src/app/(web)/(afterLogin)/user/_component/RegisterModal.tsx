...
import { B9001AData } from "@/type/api";
...
export default function RegisterModal({ session, onClose }: ModalProps<RegisterModalProps>) {
  ...
  const mutateRegister = useMutation({
    mutationKey: ["B9001A"],
    mutationFn: async () => {
      const B9001ARes = await callTms<B9001AData>({
        session,
        svcId: "B9001A",
        data: [
      ...
      const B9001AData = B9001ARes.svcRspnData;

      if (!B9001AData) throw new Error("B9001AData is null");

      return {
        id: B9001AData[0].F01,
        password: B9001AData[0].F02,
      };
    },
    onSuccess: async (data) => {
      setUserList((pre) => ({ ...pre, nonce: pre.nonce + 1 }));
      await modalStore.push(RegisterCompleteModal, {
        props: { id: data.id, password: data.password },
      });
      onClose();
    },
    ...
    <Modal id={ID} onClose={onClose}>
      <ModalCenterContent>
        ...
        <Form onSubmit={onSubmit}>
          ...
          <List>
            <InputBox>
              <Label className="essential" htmlFor={RegisterInput.id}>
                사용자 ID
              </Label>
              <Input value={id} onChange={onChange} id={RegisterInput.id} type="text" required />
            </InputBox>
            <InputBox>
          ...
          <ButtonBox>
            <ModalButton type="submit" disabled={mutateRegister.isPending}>
              {mutateRegister.isPending ? <DotsLoading /> : "등록"}
            </ModalButton>
          </ButtonBox>
        </Form>
      </ModalCenterContent>
